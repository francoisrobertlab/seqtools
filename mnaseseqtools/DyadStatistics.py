...
def dyadstatistics(output, verbose):
    '''Creates statistics file for dyads.'''
    logging.basicConfig(filename='debug.log', level=logging.DEBUG, format='%(asctime)s %(levelname)-8s %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
    dyad_statistics(output, verbose)


def dyad_statistics(output='dyad_statistics.txt', verbose=False):
    '''Creates statistics file for dyads.'''
    ...
    for i in range(0, len(all_genes_files)):
        ...
        position_headers = list(filter(re.compile('dyad position -?\\d*').match, genes.columns))
